...
class TextClass(object):
    ...
    def AddCmsSimulationPhaseTwoText(self, bIsTH2, xPos=None, yPos=None, text=None):
        self.Verbose()

        if bIsTH2 == False:
            (_x, _y, _text) = self._GetValues("cmsSimulationPhaseTwo", xPos, yPos, text)
        else:
            (_x, _y, _text) = self._GetValues("cmsSimulationPhaseTwo", xPos, yPos, text)
        (_x2, _y2, _text2) = self._GetValues("PU140", xPos, yPos, text)
