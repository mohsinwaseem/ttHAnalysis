...
class StyleClass(object):
    def __init__(self, verbose = False):
        ...
        #self.colourPaletteList      = [1, 2, ROOT.kGreen+2, ROOT.kViolet-5, ROOT.kPink-8, ROOT.kTeal-1, ROOT.kCyan-7] #TP_2014
        ...
        #self.fillStyleCounterList   = [0, 1001, 3001, 3002, 3003, 3004, 3005, 3006, 3007, 3144, 3244, 3444]
    ...
    def _GetTH1Values(self, styleType, NumOfDatasets):
        self.Verbose()
        ...
        if (NumOfDatasets == 1):
            ...
            markerStyle = getattr(self, styleType + "_markerStyle" ) #self.markerStyleCounter[styleType].next()
        markerSize  = 1.0
        lineColour  = fillColour #ROOT.kBlack
    ...
    def GetTH1Styles(self, histoObject, NumOfDatasets):
        '''
        ...
        if histoObject.styleType != None:
            styleType = histoObject.styleType
        else:
            #styleType = self.TextObject.ConvertLatexToText(histoObject.dataset).lower()
            styleType = histoObject.dataset.lower()

        if styleType in self.styleTypeList:
            return self._GetTH1Values(styleType, NumOfDatasets)
        elif styleType in self.styleTypeSpecialList:
            return self._GetTH1SpecialValues(styleType)
        else:
            raise Exception("Unknown style name '%s'. Please select one of the following style names:\n\t%s" % (styleType, "\n\t".join(self.styleTypeList)) )
    ...
    def GetTGraphStyles(self, styleType, dataset = None):
        ...
        if dataset == None:
            if (styleType == None or styleType == "") and dataset == None:
                raise Exception("Could not find the style type (styleType = '%s'). Please select one of the following style names:\n '%s" % (styleType, "' , '".join(self.styleTypeList) + "'") ) 
            else:
                pass
        else:
