...
class TH1orTH2:    
    def __init__(self, path, name, legTitle, saveName, **kwargs):
        ...
        self.yLegMin         = kwargs.get("yLegMin", 0.84)
        ...
        self.datasetLatex    = kwargs.get("dataset", None)
        self.zLabel          = kwargs.get("zLabel", "z-label")
        self.xLabel          = kwargs.get("xLabel", "x-label") + " " + self.xUnits
        self.yLabel          = kwargs.get("yLabel", "Entries / %0.0f")
        self.zLabel          = self.zLabel + " " + self.zUnits
        ...
        self.histoRangeIntegral = -1.0
        self.inclusiveIntegral  = -1.0
        self.TH1orTH2        = None
        self.TFileName       = None
        self.treeVarExp      = name
        self.treeCuts        = kwargs.get("treeCuts", None)
        self.kwargs          = kwargs
    ...
    def ApplyStyles(self, styleObject, histoObjectType, NumOfDatasets):
        '''
        ...
        self._SetFillLineMarkerStyles(styleObject, histoObjectType, NumOfDatasets)
    ...
    def _SetAxisStyle(self, histoObjectType):
        ...
        if "%" not in self.yLabel:
            self.Print(["WARNING! No provision for y-units provided in yLabel(='%s'). " % (self.yLabel) ])
        ...
        if "TH2" in str(histoObjectType):
            ...
            self.TH1orTH2.GetZaxis().SetTitleOffset(1.35)
            if (self.zMax != None):
                zMax = self.zMax
    ...
    def _SetFillLineMarkerStyles(self, styleObject, histoObjectType, NumOfDatasets):
        '''
        ...
        if "TH1" in str(histoObjectType):
            (fillColour, lineColour, markerStyle, markerSize, lineWidth, lineStyle, fillStyle, drawOptions, legOptions) = styleObject.GetTH1Styles(self, NumOfDatasets)
        elif "TH2" in str(histoObjectType):
            ...
            print "str(histoObjectType) = ", str(histoObjectType)
    ...
    def _RebinXToWidth(self, histoObjectType):
        ...
        if self.TH1orTH2.GetXaxis().GetBinWidth(0)!=self.binWidthX:
            self.Print(["WARNING! Could achieve bin-width of '%s' for x-axis of hist '%s'. The new bin-width will instead be '%f'." % ( self.binWidthX,  self.name, self.TH1orTH2.GetXaxis().GetBinWidth(0) )])
