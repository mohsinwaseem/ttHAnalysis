...
class Plotter(object): 
    ...
    def _CreateCanvas(self):
        ...
        canvasName =  canvasName + "@" + str(self.time.GetTime())
        self.TCanvas = ROOT.TCanvas( canvasName, canvasName, 1)
        time.sleep(0.5) #to avoid getting the same time (canvas/histo overwrite)
    ...
    def DrawEfficiency(self, cutDirection=">", errType="binomial"):
        ...
        kwargs["drawOptions"] = "AP" #"ACPE3"
        kwargs["legOptions"]  = "LP" #"FLP"
    ...
    def _AppendXYCutBoxesToTBoxList(self, axis):
        ...
        for v in cBoxes:
            ...
            #self.TLegend.AddEntry( cLine1, "%s #leq x #leq %s" % (str(v[0]), str(v[1])), "FL" )
            ###self.TLegend.AddEntry( cLine1, "%s #leq x #leq %s" % (str(v[0]) + self.THDumbie.xUnits, str(v[1]) + self.THDumbie.xUnits), "FL" )
            self.TLegend.SetY1( self.TLegend.GetY1() - 0.02)
    ...
    def _GetLegEntryLabel(self, histoObject):
        ...
        if self.UseDatasetAsLegEntry==True:
            ...
            #entryLabel = self.DatasetToLatexNameMap[histoObject.dataset] + ":" + histoObject.legTitle
    ...
    def AppendToTBoxList(self, box, boxColour=18):
        ...
        box.SetFillStyle(3003) #1001, 3002
    ...
    def _GetLegendHeader(self):
        ...
        ### Create TLegend name
        if self.GetNumberOfHistosPerDataset()==1: 
            header = self.DatasetObject.ConvertDatasetToLatex(histo.dataset)
        else:
            header = histo.legTitle
        return header
